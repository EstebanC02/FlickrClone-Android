...
public class PhotoUpdateService extends IntentService {
    ...
    private void performInsert(ContentValues values) {
        if (getContentResolver().insert(FlickrContract.PhotoListEntry.CONTENT_URI, values) != null)
            Log.d(TAG, "Inserted new photo");
        else
            Log.w(TAG, "Error inserting new photo");
    }

    private void performInsert(ContentValues[] values) {
        if (getContentResolver().bulkInsert(FlickrContract.PhotoListEntry.CONTENT_URI, values) > 0)
            Log.d(TAG, "Inserted new photos");
        else
            Log.w(TAG, "Error inserting new photos");
    }
    ...
    private void performUpdate(Uri uri, ContentValues values) {
        ...
        String[] selectionArgs = new String[]{String.valueOf(values.getAsString(FlickrContract.PhotoListEntry._ID))};
        int count = getContentResolver().update(uri, values, selection, selectionArgs);
        Log.d(TAG, "Updated " + count + " photo items");
    }

    private void performDelete(Uri uri) {
        int count = getContentResolver().delete(uri, null, null);
        Log.d(TAG, "Deleted " + count + " photo");
    }
}
